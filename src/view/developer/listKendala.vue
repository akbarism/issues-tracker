<template>
  <div>
    <div class="row justify-between mb-3">
      <p class="text-weight-bold ml-1 q-mb-none">Daftar Kendala</p>
      <q-btn
        size="sm"
        color="primary"
        unelevated
...
const createData = async () => {
  vm.loading = true;
  let id = `kendala-${day().unix()}`;
  await setDoc(doc(db, "kendala", id), {
    id: id,
    name: vm.name,
...
const updateData = async () => {
  vm.loading = true;
  await updateDoc(doc(db, "kendala", vm.item.id), {
    name: vm.name,
  });
  vm.dialog = false;
  fetchData();
...
const deleteData = async () => {
  await deleteDoc(doc(db, "kendala", vm.item.id));
  fetchData();
};
onMounted(() => {
  fetchData();
